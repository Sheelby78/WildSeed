...
- `healthy` — no CRITICAL/HIGH audit findings, test runner working, no high-severity config gaps. Missing CI/AGENTS.md does not affect this verdict.
- `needs-attention` — some findings, all addressable. Typical: a few HIGH advisories, missing formatter, missing type strictness.
- `critical-issues` — CRITICAL audit findings, no test runner, or compounding high-severity gaps.
...
```
No stack-assessment.md found. Run /stack-assess for quality-gate analysis.
```
