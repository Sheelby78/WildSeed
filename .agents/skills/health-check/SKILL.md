...
description: >
  ...
  /stack-assess (brownfield chain), BEFORE agent onboarding.
---

# Health Check: Audit an Existing Project for Agent-Readiness

This skill is the brownfield counterpart to `/bootstrapper`. Where bootstrapper scaffolds a new project and verifies the scaffold, health-check runs the same three execution gates (pre/in/post) as an assessment framework for an existing codebase. It reuses the per-language audit dispatch pattern from bootstrapper's post-scaffold verification but applies it as the opening move, not the closing check.

The skill sits in the brownfield chain: `/shape → /prd → /stack-assess → /health-check`. Its single job: audit the project's dependency health, test infrastructure, CI/CD configuration, and configuration completeness, then produce a structured report with prioritized fixes and an agent-readiness verdict.

When `context/foundation/stack-assessment.md` exists (from `/stack-assess`), health-check links its findings to the quality-gate gaps identified there. The two reports are complementary: stack-assess evaluates the *stack choice* against quality gates; health-check evaluates the *project state* against operational health criteria.
...
**Skip when**: the user is scaffolding a new project — `/bootstrapper` runs its own verification slots. Skip also when the user only wants a stack quality-gate assessment without operational health checks — that is `/stack-assess` territory.

## Relationship to other skills

- `/stack-assess` — upstream. Produces `context/foundation/stack-assessment.md`. Optional input — health-check can run without it, but the report is richer when gaps are linked.
- `/bootstrapper` — greenfield parallel. Same three execution gates, different application (scaffold verification vs existing project audit).
- `/shape`, `/prd` — earlier in the brownfield chain. Not direct inputs, but the PRD's scope-of-change context can inform which parts of the project matter most.
...
1. **If a path argument was provided** (e.g. `/health-check @context/foundation/stack-assessment.md`), strip a leading `@` if present and use the path as the stack-assessment location for this run. The assessment is optional context, not a precondition.
...
```
No project markers found in the current directory. /health-check requires an existing codebase.
If you're starting from scratch, use /bootstrapper after /tech-stack-selector instead.
```

Then STOP.

If markers are found, detect the language family from the marker (same detection logic as `/stack-assess` Step 1) and proceed to Step 1.
...
| `CLAUDE.md` / `AGENTS.md` | Agent instruction files | low |
...
- **healthy**: no CRITICAL/HIGH audit findings, test runner detected and working, no high-severity configuration gaps.
- **needs-attention**: some findings but all addressable. Typical: a few HIGH audit advisories, missing formatter, or missing type strictness.
- **critical-issues**: CRITICAL audit findings, no test runner, or multiple high-severity gaps compounding. The AI assistant will struggle without preparation.

Findings like missing CI, missing AGENTS.md, or missing deployment config do **not** affect the verdict. A project can be `healthy` with no CI pipeline if it has a working test runner, clean dependencies, and good local configuration.

The verdict is informational, not blocking. Even `critical-issues` means "invest time in these fixes before expecting smooth AI assistant collaboration," not "abandon the project."

#### 3c. Prioritized fix list

**Fix before AI assistant work** (actionable now):
...
6. **Cross-reference stack-assessment when available.** If the user ran `/stack-assess` first, health-check must link findings to quality-gate gaps. The two reports are complementary — don't duplicate the gate analysis, reference it.
