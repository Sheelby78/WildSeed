...
description: >
  ...
  PRD", "PRD from shape-notes". Use AFTER /shape, not in place of it.
---

# PRD: Generate context/foundation/prd.md from shape-notes

This skill is the second link in the bootstrap chain. For greenfield: `/shape → /prd → tech-stack-selector → bootstrapper`. For brownfield: `/shape → /prd → stack-assess → health-check`. Its single job: take a shaped notes file and emit a `context/foundation/prd.md` that conforms to the locked PRD schema, routing every gap to `## Open Questions` rather than inventing content.
...
The locked schema this skill conforms to lives at `../shape/references/prd-schema.md` (relative to this SKILL.md). Read it before generating any artifact and re-check the produced file against it before writing to disk.

## When to use, when to skip

**Use when**: the user has run `/shape` (and `context/foundation/shape-notes.md` exists with a checkpoint block), OR the user has a raw notes file they want turned into a PRD draft, OR the user explicitly asks to (re-)generate `context/foundation/prd.md`.

**Skip when**: the user is still ideating and has no notes — point at `/shape` first. Skip also when the user wants to *edit* an existing PRD by hand — this skill writes whole files; surgical edits are out of scope.

## Relationship to other skills

- `/shape` — produces `shape-notes.md`, the canonical input. Always preferred upstream of this skill.
- `tech-stack-selector` — downstream consumer of `prd.md` for **greenfield**. Reads the product-level frontmatter as priors and runs its own residual interview for team composition, language preferences, deployment, and CI/CD shape.
- `stack-assess` — downstream consumer of `prd.md` for **brownfield**. Evaluates the existing stack against agent-friendly quality gates.
- `/frame`, `/plan` — unrelated; PRD is a foundation artifact, not a per-change plan.

## Initial Response

When this skill is invoked:

1. **If a path argument was provided** (e.g. `/prd @notes/raw.md` or `/prd context/foundation/shape-notes.md`), capture it as the input path. Proceed to Step 1.
...
- "Run /shape first (Recommended)" (description: "Stop here. Run /shape to produce shape-notes.md, then re-invoke /prd.")
...
On "Run /shape first": print "Stopping. Run `/shape` to produce shape-notes.md, then re-invoke `/prd`." and STOP.
...
2. **If no `context_type:` in frontmatter** (raw notes, pasted input) — auto-detect from cwd:
   ...
   Use the same multi-signal detection as `/shape` (Step 0.7): check for git history (Tier 1), lockfiles (Tier 2), manifest files (Tier 3), and bonus signals (source dirs, framework configs). Any Tier 1 or Tier 2 hit → propose brownfield. Tier 3 only → propose brownfield with ambiguity flag. No signals → propose greenfield.
...
1. **Frontmatter `checkpoint:` block present** — strongest signal that this came from `/shape`. Look for the literal `checkpoint:` key inside a YAML frontmatter fence at the top of the file.
...
1. **Frontmatter `checkpoint:` block present AND `context_type: brownfield`** — strongest signal that this came from `/shape` in brownfield mode. Also check for `## Current System` section in the body.
...
- "Run /shape first (Recommended)" (description: "Stop here. Use /shape to fill in the missing signals, then re-invoke /prd.")
- "Proceed anyway" (description: "Generate the PRD from what's there. Missing pieces land in ## Open Questions verbatim.")
- "Cancel" (description: "Exit without changes.")

On "Run /shape first": print the redirect message and STOP. On "Proceed anyway": continue to Step 3 with `score < 2` recorded so later steps know to expect TODOs. On "Cancel": STOP.

### Step 3: Generate PRD

Read the schema reference FULLY one more time (`../shape/references/prd-schema.md`) to confirm the field list and section names have not drifted.
...
If `/shape` recorded Socrates blockquotes under FRs, preserve them verbatim — they're load-bearing for downstream review.

If shape-notes.md carried a `## Quality cross-check` block (from Step 7 of `/shape`), mirror each gap into `## Open Questions` as a numbered entry naming the missing element and its consequence.
...
have drifted — re-read ../shape/references/prd-schema.md and reconcile.
...
```bash
echo -n "/tech-stack-selector" | pbcopy 2>/dev/null || echo -n "/tech-stack-selector" | clip.exe 2>/dev/null || echo -n "/tech-stack-selector" | xclip -selection clipboard 2>/dev/null || true
```

```powershell
# PowerShell (Windows)
Set-Clipboard "/tech-stack-selector"
```
...
```bash
echo -n "/stack-assess" | pbcopy 2>/dev/null || echo -n "/stack-assess" | clip.exe 2>/dev/null || echo -n "/stack-assess" | xclip -selection clipboard 2>/dev/null || true
```

```powershell
# PowerShell (Windows)
Set-Clipboard "/stack-assess"
```

```
► Next:   /stack-assess  (✓ copied to clipboard)

          It evaluates your existing stack against agent-friendly
          quality gates and produces a compensation plan. After that,
          /health-check audits dependency health, test suite,
...
2. **Schema is the contract.** `../shape/references/prd-schema.md` defines frontmatter keys, section names, and section order. Re-read it at every invocation. Re-validate the in-memory PRD against it in Step 3c before writing. Drift between this skill and the schema is the failure mode this skill exists to prevent.
...
7. **Never chain automatically.** The hand-off is an announcement, not an invocation. The user picks when (and whether) to run the next step (tech-stack-selector for greenfield, stack-assess for brownfield). Auto-chaining would skip the human's review of the generated PRD.

## Notes

- This is a **document generator** skill. Output is `context/foundation/prd.md` (or `prd-vN.md`), period.
- The schema reference (`../shape/references/prd-schema.md`) is the single source of truth. Any field name, section name, or frontmatter key referenced in this body MUST exist in the schema doc — if it doesn't, fix the schema doc first.
...
- The `# TODO: <field-name> — see Open Questions` pattern is load-bearing. Downstream tooling (review skills, tech-stack-selector / stack-assess) can grep for `^# TODO: ` to count unresolved gaps and decide whether the PRD is review-ready.