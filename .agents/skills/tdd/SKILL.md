---
name: tdd
description: Drive an approved plan from context/changes/<change-id>/plan.md phase by phase, test-first, through red→green→refactor — only for TDD'able phases not yet implemented; everything else routes to /implement. Use when the user says "tdd", "test-first", "red green refactor", or wants to execute a plan via TDD.
---
...
After all phases →  Completion summary + optional /impl-review
```
...
1. **Resolve the plan**:
   - `/tdd <change-id> [phase N]` → `context/changes/<change-id>/plan.md`.
   - `@context/changes/<change-id>/plan.md` or a full path → accept as-is.
   - **Refuse if the resolved path starts with `context/archive/`** — print "This change is archived. Open a new change with `/new` instead." and STOP.
...
1. A change-id (e.g., `/tdd oauth-login phase 1`), or
...
This skill assumes test infrastructure already exists; it won't set it up. Options:
  • Set up a runner first, then re-run /tdd.
  • Use /implement to build the plan without test-first.
  • Use /test-plan for a phased test-rollout strategy.
```
...
Use /implement to proceed with this phase:
→ /implement <change-id> phase [N]
```

Copy `/implement <change-id> phase [N]` to the clipboard per the clipboard convention, display it with `(✓ copied)` when successful, and STOP. `/implement` can continue the phase from the existing code and plan state.
...
| TDD'able — drive it here | Not TDD'able — redirect to `/implement` |
|---|---|
...
- If it's **mixed or ambiguous** (e.g., a phase that scaffolds a config *and* adds a validator with real logic), ask the user: "Phase [N] is partly scaffolding, partly logic. How should I drive it?" with options:
  ...
  - "Redirect whole phase to /implement": "Hand the entire phase off — copy the resume command to the clipboard."
  - "TDD the whole phase anyway": "Force test-first even for the thin parts. May produce low-value tests."

### Redirect a non-TDD'able phase to `/implement`
...
State *why* the phase isn't a fit (one or two sentences, grounded in the table above), then ask the user: "Phase [N] isn't a good test-first fit. How do you want to handle it?" with options:
  - "Hand off to /implement (Recommended)": "Copy `/implement <change-id> phase N` to the clipboard. Clear context, run it, then resume TDD on the next phase."
...
**On "Hand off":** copy `/implement <change-id> phase [N]` to the clipboard (per the clipboard convention), print the block below, and STOP — `/implement` will flip this phase's Progress rows and run its own commit ritual. Tell the user to resume TDD afterward.

```
Phase [N] isn't test-first material — [one-line reason].

→ /implement <change-id> phase [N] (✓ copied)

Clear context (`/clear`), run that, then come back with:
→ /tdd <change-id> phase [N+1]
```
...
When all `#### Automated` rows in `### Phase N:` are `[x]`, run the phase-end ritual (this mirrors `/implement` — one Conventional-Commits commit per phase, then write its short SHA back into the rows that flipped).
...
8. **Capture and write back the SHA.** `git rev-parse --short HEAD` → `SHA`. For every Progress row flipped this phase, modify `- [x] N.M <title>` → `- [x] N.M <title> — <SHA>` (skip rows that already carry a SHA; if `SHA=""`, skip — `/archive` surfaces SHA-less rows as informational warnings).
...
Ask the user: "Phase [N] complete (test-first). How to proceed?" with options:
  ...
  - "Review this phase first": "Run /impl-review to verify the implementation against the plan before continuing."

**Continue:** read the next phase, set its task `in_progress`, run the TDD gate, proceed. No need to re-read the whole plan.

**Review:** run `/impl-review @<path-to-plan> phase [N]`, then re-present the continue/clear decision (without the review option).

**Clear:** copy `/tdd <change-id> phase [N+1]` to the clipboard (per the clipboard convention) and display it as `→ /tdd <change-id> phase [N+1] (✓ copied)`.
...
2. **Update `change.md`**: `status: implemented`, `updated: <today>`. (Do NOT set `archived_at` — that's `/archive`.)
...
Summary:
- Phases completed: [N]  ([k] TDD'd, [j] redirected to /implement)
- Tests added: [count] across [files]
- Files changed: [key files]
```

   Then ask the user: run `/impl-review <change-id>` (full-plan review) or skip.
