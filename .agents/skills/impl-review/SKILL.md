...
**Every path through this skill persists the report and stamps the change** — Triage now, Triage later, and Done all write the file. This is what lets `/archive` and `/status` see the review and keeps `change.md.status` correct. Do this *before* presenting the proceed options — never conditionally, and never only on the "save" branches.
...
options:
  ...
  - label: "Triage later"
    description: "Resume with /impl-review <report-path>."
  - label: "Done"
...
- **Triage later** → print the saved report path and remind them to run `/impl-review <report-path>`.
...
options:
  ...
  - label: "Record as lesson"
    description: "Save as a recurring project rule via /lesson."
multiSelect: false
