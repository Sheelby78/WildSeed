...
1. **Resolve the change folder, then write the plan** to `context/changes/<change-id>/plan.md`.
   - If the user invoked `/plan <change-id>` and `context/changes/<change-id>/` already exists, use it.
   - Otherwise derive a kebab-case `<change-id>` from the topic and create the folder + `change.md` (mirroring `/new` semantics) before writing.
...
2. **Copy quick start command to clipboard**:
   ...
   ```bash
   echo -n "/implement <change-id> phase 1" | pbcopy 2>/dev/null || echo -n "/implement <change-id> phase 1" | clip.exe 2>/dev/null || echo -n "/implement <change-id> phase 1" | xclip -selection clipboard 2>/dev/null || true
   ```

   ```powershell
   # PowerShell (Windows)
   Set-Clipboard "/implement <change-id> phase 1"
   ```
...
- **If context feels degraded during planning** — if responses become sluggish or repetitive, save the current plan draft to file and offer the user to continue in a fresh context:
  ...
  → /plan <change-id> (✓ copied)
  ```
  This lets `/plan` reload the draft and continue iterating with full context available.
