...
This is the canonical structure for the test plan produced by `/test-plan`. The file is **both a phased rollout strategy and a growing cookbook**:
...
- **Cookbook** (§6) starts as placeholders and **fills in incrementally** as rollout phases ship. Each plan's final sub-phase (per the `/plan` constraint baked in by the orchestrator) updates the cookbook with patterns the rollout phase just delivered.
...
3. **Risks are scenarios, not code locations.** This plan documents *what
   ...
   produced by `/research` during each rollout phase. If the plan and
...
Short by design. Tone is imperative — these are the rules every downstream phase obeys. Principle #3 is the load-bearing distinction between this plan (QA spec) and `/research` (knowledge extraction).
...
`/research` during each rollout phase. If the plan tries to cite them
...
is the durable handoff from risk analysis to `/research` and
`/plan`.
...
via `/new`. Status moves left-to-right through the values below; the
...
those belong in per-phase `/research`.
```
...
**This is the section that fills in over time.** The orchestrator writes placeholders during Phase 4; each rollout phase's plan updates the relevant sub-section in its final sub-phase (per the `/plan` constraint).
...
(Optional. After each phase lands, /implement appends a 2-3 line note
...
## How `/test-plan` maintains this file
...
- **After each rollout phase completes** (via `/implement`'s final sub-phase, baked into the plan by the orchestrator's planning prompt): the relevant §6 sub-section is filled in; §8 dates may bump if tools were verified.
