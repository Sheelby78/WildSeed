...
description: >
  ...
  the input to /prd. Trigger phrases: "new project", "from scratch",
  "od pomysłu", "shape an idea", "I have an idea", "greenfield",
  "brownfield", "istniejący projekt", "zmiana w projekcie".
  Use BEFORE /prd, not in place of it.
---

# Shape: Facilitate Discovery (Greenfield & Brownfield) Before /prd

This skill is the head of the bootstrap chain. For greenfield: `/shape → /prd → tech-stack-selector → bootstrapper`. For brownfield: `/shape → /prd → stack-assess → health-check`. Its single job: walk a user from "I have an idea" (greenfield) or "I want to change this system" (brownfield) to a structured `context/foundation/shape-notes.md` that `/prd` can turn into a PRD that conforms to the locked schema.
...
The locked schema both this skill and `/prd` conform to lives at `references/prd-schema.md` (relative to this SKILL.md). Read it before producing any artifact and re-check against it at every checkpoint write.
...
**Skip when**: the project already has a PRD or an ADR set (use `/frame` or `/plan` instead), or the user is reasoning about a single bug / refactor / small feature within an existing codebase that doesn't warrant a full PRD (use `/frame`). For brownfield projects where the user wants to shape a meaningful change, this skill IS the right starting point.

## Relationship to other skills

- `/init` — scaffolds the `/context` skeleton (`changes/`, `archive/`, `foundation/`) plus universal READMEs in each. `/shape` requires `context/foundation/` to exist; if absent, it delegates to `/init` via a tool call (Step 0 below).
- `/prd` — consumes `shape-notes.md`. The handoff is the `## Step 8` clipboard write.
- `/frame` — for *reframing* small-scope problems within existing systems where a full PRD is overkill. `/shape` is for larger brownfield changes (new modules, significant features) that need structured discovery and a PRD.
- `/stack-assess` — downstream of `/prd` for brownfield projects. Evaluates existing stack against quality gates.
- `/health-check` — downstream of `/stack-assess` for brownfield. Audits existing project health.
- `/plan` — downstream of `/prd`, never invoked from here directly.

## Initial Response

When this skill is invoked:

1. **If a freeform idea was provided as the argument** (e.g. `/shape a recipe app that suggests meals from what's in your fridge`), capture it verbatim as the **seed idea**. Do not rephrase. Proceed to Step 0.
2. **If a file path was provided** (e.g. `/shape @notes/idea.md`), read it FULLY and use its contents as the seed. Proceed to Step 0.
...
Tip: pass the idea inline — `/shape a recipe app that uses fridge contents`
     or for brownfield — `/shape add a recommendation engine to my recipe app`
```
...
"This directory isn't initialized for 10xWorkflow (context/foundation/ is missing). Run /init now?" with options:
- "Yes — run /init (Recommended)" (description: "Scaffolds the /context skeleton (changes/, archive/, foundation/) with READMEs, then continues shaping.")
- "No — stop here" (description: "Exit without changes. You'll need to initialize before shape can run.")

On "Yes": invoke `/init` via a tool call (NOT via Bash). When `/init` returns, re-check the precondition; if it now passes, continue to Step 0.5. On "No": print "Stopping. Run `/init` when ready, then re-invoke `/shape`." and STOP.

Do not duplicate `/init`'s scaffold logic. A tool call to the skill is the correct delegation path.
...
Write the confirmed `context_type` into the shape-notes.md frontmatter (alongside `checkpoint:`) immediately. This value is load-bearing for `/prd`'s auto-routing.
...
- NEVER pre-commit to a stack (framework, database, hosting platform, language family). The PRD captures product-level priors only — `product_type`, `target_scale`, `timeline_budget`. Stack-shaped concerns are gathered downstream of `/prd`.
...
- "Accept and finish" (description: "Proceed despite the gaps. They will be recorded as warnings in the checkpoint and surfaced in /prd's Open Questions.")
...
```bash
echo -n "/prd" | pbcopy 2>/dev/null || echo -n "/prd" | clip.exe 2>/dev/null || echo -n "/prd" | xclip -selection clipboard 2>/dev/null || true
```
...
═══════════════════════════════════════════════════════════
  ...
  After /prd, the next chain step will pick up:
...
STOP. Do not chain into `/prd` automatically — the user runs it when ready.
...
5. **Soft gate, not hard gate.** The closing cross-check WARNS but allows the user to override every gap. Override paths are recorded in the checkpoint as `quality_check_status: warned` and surfaced in `/prd`'s `## Open Questions`. Refusing to finish is not in scope.

6. **Mode-aware behavior.** The skill auto-detects context type (greenfield vs brownfield) from project markers in cwd and adapts all six discovery phases accordingly. For brownfield, the discovery loop shifts from "what are you building from scratch?" to "what exists, what's changing, what must be preserved?". If the user invokes this skill for a small-scope problem within an existing codebase (single bug, quick refactor), suggest `/frame` instead — `/shape` is for changes that warrant a full PRD.
...
- This is a **shaping** skill. Output is `shape-notes.md`, not `prd.md`. `/prd` is the document generator.
