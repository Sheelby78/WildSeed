...
This skill is the chain-tail of the bootstrap sequence (`/shape → /prd → /tech-stack-selector → bootstrapper`). Its single job: turn a written tech-stack hand-off into a scaffolded project in the current working directory, with verification findings logged for the user to review.

The skill is a **registry consumer**, not a registry owner. The starter registry lives in `/tech-stack-selector` (`/skills/tech-stack-selector/references/starter-registry.yaml`); bootstrapper looks up the chosen card by `starter_id`, substitutes its `cmd_template`, and dispatches to the right cwd strategy. A CI validator (`scripts/validate-starter-registry-sync.mjs`) prevents bootstrapper from referencing a `starter_id` absent from that registry.


## When to trigger

Use when `context/foundation/tech-stack.md` exists and the user is ready to scaffold. Trigger phrases: "bootstrap the project", "scaffold the app", "set up the codebase", "let's start the project", "spin up the repo", or any natural follow-up to a `/tech-stack-selector` run that just wrote the hand-off.
...
- The user is mid-implementation on an existing codebase asking to add a single library or replace a single dependency — that is `/frame` territory, not bootstrap.
- The user names a stack outside the tech-stack-selector registry — redirect to `/tech-stack-selector` (it owns the registry; if a starter is missing, that is where it lands).
- `context/foundation/tech-stack.md` is absent — the precondition check at Step 0 handles this with an explicit redirect.

## Required inputs

1. `context/foundation/tech-stack.md` — the hand-off written by `/tech-stack-selector`. Contract: see `references/handoff-consumer.md` (which pins to `/tech-stack-selector/references/handoff-schema.md` as the authoritative schema).
2. The chosen card from `/skills/tech-stack-selector/references/starter-registry.yaml`. Resolved by `starter_id` lookup. Carries `cmd_template`, `language_family`, `bootstrapper_confidence`, `toolchain.package_manager`, `deployment_defaults`.
...
1. **If a path argument was provided** (e.g. `/bootstrapper @context/foundation/tech-stack-v2.md` or `/bootstrapper path/to/tech-stack.md`), strip a leading `@` if present and use the path verbatim as the hand-off location for this run.
...
```
Bootstrapper requires a tech-stack hand-off at `<handoff-path>`. Run `/tech-stack-selector` first, then re-invoke.
```
...
**If present**, read it FULLY (no `limit`/`offset`) and proceed. Parse the frontmatter per `references/handoff-consumer.md` and resolve the chosen card by `starter_id` lookup against `/skills/tech-stack-selector/references/starter-registry.yaml`. If the lookup fails, run the registry-drift refusal from `references/refusal-protocol.md` and STOP.
...
- question: "Proceed with this hand-off, or correct something first?"
  ...
  - label: "Stop — fix the hand-off first"
    description: "Exit. Re-run /tech-stack-selector to update tech-stack.md, then re-invoke."
  multiSelect: false
...
4. **CLI failure is HARD-STOP.** If the exit code is non-zero, run the CLI failure handling path in `scaffold-merge.md` § CLI failure handling: leave `.bootstrap-scaffold/` in place, do not apply the conflict matrix, write a partial `verification.md` with `phase_3_status: failed`, set the clipboard to `/bootstrapper`, print the failure summary, and STOP. Do not advance to Step 3.
...
For the HARD-STOP partial-log case (Step 2 CLI failure), Step 4 still runs but with the truncated body shape in the schema (`Audit not run` section, `phase_3_status: failed`). The clipboard is set to `/bootstrapper` for retry by Step 2's failure path, not by this step.
...
- **Clipboard pointer on failure paths only** — `/tech-stack-selector` for the missing-handoff and registry-drift refusals, `/bootstrapper` for the Step 2 CLI-failure HARD-STOP retry. No clipboard set on a successful run.
...
2. **Bootstrapper consumes the registry; it does not own it.** The canonical starter registry lives in `/tech-stack-selector`. Drift between bootstrapper-referenced `starter_id`s and the registry is a CI failure (`scripts/validate-starter-registry-sync.mjs`).
