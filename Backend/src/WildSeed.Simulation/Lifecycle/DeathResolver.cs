...
public static class DeathResolver
{
    public static IReadOnlyList<SimulationEvent> Resolve(SimulationState state)
    {
        ...
        {
            DeathCause? cause = organism.Needs.Thirst >= OrganismNeeds.Maximum ? DeathCause.Dehydration : organism.Needs.Hunger >= OrganismNeeds.Maximum ? DeathCause.Starvation : organism.AgeTicks >= SurvivalRulesV3.MaximumAgeTicks ? DeathCause.OldAge : null;
