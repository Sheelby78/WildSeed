...
checkpoint:
  ...
  gray_areas_resolved:
    ...
    - topic: primary persona scope
      decision: single named user — the creator developing a portfolio project
    - topic: access model
      decision: public demo without login; each visitor can run their own simulation
    - topic: MVP timeline
      decision: four weeks of intensive work, not limited to after-hours; sustained-effort cost accepted
    - topic: secondary success criterion
      decision: Limited God Mode
    - topic: MVP guardrails
      decision: deterministic simulation and a rendering-independent simulation engine
    - topic: simulation capacity
      decision: at least 5,000 simultaneous organisms on a typical modern computer with stable operation
    - topic: MAX performance
      decision: at least 20x realtime with 5,000 active organisms and rendering disabled
    - topic: product type
      decision: web app
    - topic: target audience scale
      decision: a handful of users; simulation rules remain unchanged at 100x that audience and each visitor runs an independent world
    - topic: hard deadline
      decision: no hard deadline
    - topic: project motivation
      decision: private hobby experiment and simulation/game curiosity that also demonstrates programming ability; it is not intended to solve an external user's problem
    - topic: MVP finish quality
      decision: every capability included in MVP is delivered in a polished, final-style form without placeholder or default browser UI
  frs_drafted: 12
...
Seed source: `context/foundation/brief.md` (read in full).
...
Acknowledged on 2026-08-26: 4-week MVP requires sustained dedication; user accepted. The work is not limited to after-hours.
...
- Timeline-cost acknowledgment: present for the four-week intensive MVP.
- Non-Goals: present.
